...
suite('Full Member Function Tests', () => {
	...
	describe('ParseAndSerializeSingle', function() {
		...
		assert.strictEqual(await memberFnct.serialize(SerializableMode.header), 'int fncName ('+arg+');');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.source), 'int TestClass::fncName ('+arg+') {\n' +
		'\tint returnValue;\n\treturn returnValue;\n}');

		assert.strictEqual(await memberFnct.serialize(SerializableMode.implHeader), '');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.implSource), '');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.interfaceHeader), '');
	...
	describe('ParseAndSerializeSingleWithConstReturn', function() {
		...
		assert.strictEqual(await memberFnct.serialize(SerializableMode.header), 'const int* fncName ('+arg+');');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.source), 'const int* TestClass::fncName ('+arg+') {\n' +
	...
	describe('ParseAndSerializeSingleWithWhitespaceInReturn', function() {
		...
		assert.strictEqual(await memberFnct.serialize(SerializableMode.header), 'std::pair<int, void*> fncName ('+arg+');');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.source), 'std::pair<int, void*> TestClass::fncName ('+arg+') {\n' +
	...
	describe('ParseAndSerializeSingleConst', function() {
		...
		assert.strictEqual(await memberFnct.serialize(SerializableMode.header), 'int fncName ('+arg+') const;');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.source), 'int TestClass::fncName ('+arg+') const {\n' +
		'\tint returnValue;\n\treturn returnValue;\n}');
	...
	describe('ParseAndSerializeSingleVirtual', function() {
		...
		assert.strictEqual(await memberFnct.serialize(SerializableMode.header), 'int fncName ('+arg+') override;');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.source), 'int TestClass::fncName ('+arg+') {\n' +
		'\tint returnValue;\n\treturn returnValue;\n}');

		assert.strictEqual(await memberFnct.serialize(SerializableMode.implHeader), '');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.implSource), '');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.interfaceHeader), 'virtual int fncName ('+arg+') =0;');
	...
	describe('ParseAndSerializeSingleVirtualConst', function() {
		...
		assert.strictEqual(await memberFnct.serialize(SerializableMode.header), 'int fncName ('+arg+') const override;');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.source), 'int TestClass::fncName ('+arg+') const {\n' +
		'\tint returnValue;\n\treturn returnValue;\n}');

		assert.strictEqual(await memberFnct.serialize(SerializableMode.implHeader), '');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.implSource), '');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.interfaceHeader), 'virtual int fncName ('+arg+') const =0;');
		done();
		});
	});
	...
	describe('ParseAndSerializeSinglePureVirtual', function() {
		...
		assert.strictEqual(await memberFnct.serialize(SerializableMode.header), 'virtual int fncName ('+arg+') =0;');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.source), '');

		assert.strictEqual(await memberFnct.serialize(SerializableMode.implHeader), 'int fncName ('+arg+') override;');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.implSource), 'int TestClass::fncName ('+arg+') {\n' +
		'\tint returnValue;\n\treturn returnValue;\n}');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.interfaceHeader), '');
	...
	describe('ParseAndSerializeSinglePureVirtualConst', function() {
		...
		assert.strictEqual(await memberFnct.serialize(SerializableMode.header), 'virtual int fncName ('+arg+') const =0;');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.source), '');

		assert.strictEqual(await memberFnct.serialize(SerializableMode.implHeader), 'int fncName ('+arg+') const override;');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.implSource), 'int TestClass::fncName ('+arg+') const {\n' +
		'\tint returnValue;\n\treturn returnValue;\n}');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.interfaceHeader), '');
	...
	describe('ParseAndSerializeSingleStatic', function() {
		...
		assert.strictEqual(await memberFnct.serialize(SerializableMode.header), 'static int fncName ('+arg+');');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.source), 'int TestClass::fncName ('+arg+') {\n' +
		'\tint returnValue;\n\treturn returnValue;\n}');
	...
	describe('ParseMultipleMixedType', function() {
		...
		assert.strictEqual(memberFnct.returnVal, "int");
		assert.strictEqual(memberFnct.isConst, false);
		assert.strictEqual(await memberFnct.serialize(SerializableMode.header), 'virtual int fncName2 ('+arg+') =0;');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.source), '');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.implHeader), 'int fncName2 ('+arg+') override;');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.implSource), 'int TestClass::fncName2 ('+arg+') {\n' +
		'\tint returnValue;\n\treturn returnValue;\n}');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.interfaceHeader), '');
		...
		assert.strictEqual(memberFnct.isConst, false);
		assert.strictEqual(await memberFnct.serialize(SerializableMode.header), 'std::shared_ptr<Test> fncName3 ('+arg+');');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.source),  'std::shared_ptr<Test> ITestClass::fncName3 ('+arg+') {\n' +
		...
		assert.strictEqual(memberFnct.isConst, true);
		assert.strictEqual(await memberFnct.serialize(SerializableMode.header), 'void fncName4 ('+arg+') const;');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.source),  'void ITestClass::fncName4 ('+arg+') const {\n}');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.implHeader), '');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.implSource), '');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.interfaceHeader), '');
		...
		assert.strictEqual(memberFnct.returnVal, "int");
		assert.strictEqual(memberFnct.isConst, false);
		assert.strictEqual(await memberFnct.serialize(SerializableMode.header), 'int fncName5 ('+arg+') override;');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.source),  'int ITestClass::fncName5 ('+arg+') {\n' +
		'\tint returnValue;\n\treturn returnValue;\n}');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.implHeader), '');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.implSource), '');
		assert.strictEqual(await memberFnct.serialize(SerializableMode.interfaceHeader), 'virtual int fncName5 ('+arg+') =0;');
