...
suite('Matcher Tests', () => {	
	...
	test('TextFragment: RemovingRegexMatcher return correct group matches', (done) => {
		...
		for (let index = 0; index < 2; index++) {
			...
			assert.strictEqual(groupMatch.blocks[0].scopeStart, testContent.indexOf(str));
			assert.strictEqual(groupMatch.blocks[0].scopeEnd, testContent.indexOf(str)+str.length-1);
		}
